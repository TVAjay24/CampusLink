...
// Find the closing segment in app.js
const targetEndUnix = '        }\n    }\n};';
const targetEndWin = '        }\r\n    }\r\n};';

let targetIdx = appContent.lastIndexOf(targetEndWin);
let newline = '\r\n';

if (targetIdx === -1) {
    targetIdx = appContent.lastIndexOf(targetEndUnix);
    newline = '\n';
}

if (targetIdx === -1) {
    console.error('Could not find target closing pattern at the end of app.js');
    process.exit(1);
}

// Target the end of the last function before };
const insertIndex = targetIdx + (newline === '\r\n' ? 10 : 8); // index of "};"

const before = appContent.substring(0, insertIndex);
const after = appContent.substring(insertIndex);

const merged = before + ',' + newline + newMethods + after;
